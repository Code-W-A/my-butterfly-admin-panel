...
export function ChartAreaInteractive() {
  ...
  const filteredData = React.useMemo(() => {
    // Defensive: never allow Recharts to receive `data={undefined}`.
  ...
  const defaultTooltipIndex = safeData.length ? Math.min(10, safeData.length - 1) : undefined;
      ...
      <CardHeader>
        ...
        <CardAction>
          <ToggleGroup
            ...
            value={timeRange}
            onValueChange={(value) => {
              // Radix ToggleGroup (single) can emit an empty value when toggled off.
              // Keep the current selection instead of storing an empty/invalid value.
